...
Splitting the data into training and test set with 90-10 split
```{r}
index <- sample(nrow(telecom))
telecom.subset <- telecom[index,]
row.num <- round(nrow(telecom.subset) * 0.9)
...
```{r}
glm_pred <- predict(glm_model.telecom, newdata = telecom.test)
...
pred <- prediction(pred.glm.train, pred.glm.train$Churn)
